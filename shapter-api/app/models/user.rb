class User
  ...
  class << self
    def schools_for(email)
      ...
      schools << Tag.find_or_create_by(category_code: :school, name: "Centrale Lyon") if (email =~ /.*@ecl[0-9]+.ec-lyon.fr/ or email =~ /.*@auditeur.ec-lyon.fr/)

      schools << Tag.find_or_create_by(category_code: :school, name: "Centrale Paris") if (email =~ /.*@student.ecp.fr/)

      schools << Tag.find_or_create_by(category_code: :school, name: "ULM") if ( email =~ /.*@clipper.ens.fr/)

      schools << Tag.find_or_create_by(category_code: :school, name: "HEC") if (email =~ /.*@hec.edu/)

      schools << Tag.find_or_create_by(category_code: :school, name: "Ponts ParisTech") if (email =~ /.*@eleves.enpc.fr/)

      schools << Tag.find_or_create_by(category_code: :school, name: "ESPCI") if (email =~ /@bde.espci.fr/)

      schools << Tag.find_or_create_by(category_code: :school, name: "ESCP Europe") if (email =~ /@edu.escpeurope.eu/)

      schools << Tag.find_or_create_by(category_code: :school, name: "ENSMA") if (email =~ /@etu.isae-ensma.fr/)
