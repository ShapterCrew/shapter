...
module CanRecommend
  ...
  def reco_score_item!(item,score)
    raise("score should be an integer in [0,4]") unless score.is_a? Integer and score < 5 and score > -1
    if score == 0
      self.unrecommended_items.delete(item)
      self.norecommended_items.delete(item)
      self.recommended_items.delete(item)
      self.loved_items.delete(item)
    elsif score == 1
  ...
  def item_reco_score(item)
    return 4 if loved_items.include? item
